...
清理临时文件,删除编译过程中的临时文件。这是最轻量的清理，可以快速尝试。

rm -rf ./tmp

清理编译产物,清除已生成的固件镜像和根文件系统，但保留已编译好的工具链和软件源码。适用于发现配置错误后重新编译，能节省一些时间。

make clean

深度清理,在make clean的基础上，还会删除所有已解压的软件源码和内核源码。这会强制系统在下次编译时重新下载和解压所有软件包，能有效清除大部分因依赖关系残留导致的体积问题。
